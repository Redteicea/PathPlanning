...
class DStar:
    ...
    def on_press(self, event):
        ...
        if x < 0 or x > self.x - 1 or y < 0 or y > self.y - 1:
            ...
            while s_curr != self.s_goal:
                ...
                s_curr = min(s_list, key=s_list.get)    # 主体移动，向 s_goal 前进
                path.append(s_curr)

                if i < 1:   # 只执行一次重新路径规划
                    self.km += self.h(s_last, s_curr)
                    s_last = s_curr
                    if (x, y) not in self.obs:  # 添加障碍点(x, y)
                        self.obs.add((x, y))
                        plt.plot(x, y, 'sk')
    ...
    def ComputePath(self):
        while True:
            ...
            if k_old < self.CalculateKey(s):    # 检查该点的 key 是否需要更新(需要重新规划路径时)
                self.U[s] = self.CalculateKey(s)    # 需要更新则重新加入优先队列
            elif self.g[s] > self.rhs[s]:   # 局部过一致
                self.g[s] = self.rhs[s]     # 障碍物消除 或 找到更短路径
                for x in self.get_neighbor(s):      # 向前继节点传递更新
                    self.UpdateVertex(x)
            else:    # 局部欠一致，增加了障碍物（原最短路径受障碍物影响）
                self.g[s] = float("inf")
                self.UpdateVertex(s)
                for x in self.get_neighbor(s):  # 向前继节点传递更新
                    self.UpdateVertex(x)
    ...
    def UpdateVertex(self, s):
        ...
        if self.g[s] != self.rhs[s]:    # 不是局部一致状态就加入优先列表
