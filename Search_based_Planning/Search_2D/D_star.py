...
class DStar:
    ...
    def init(self):
        for i in range(self.Env.x_range):
            for j in range(self.Env.y_range):
                self.t[(i, j)] = 'NEW'  # 节点访问标志位，new:节点未被访问过；OPEN:正在访问中；CLOSED:已经访问过，从优先队列中移除
                self.k[(i, j)] = 0.0    # 记录自X节点被加入到OPEN_LIST中后的最小h(X)值（具体计算方式由Insert函数决定）,也是优先队列OPEN_LIST的排序依据。
                self.h[(i, j)] = float("inf")   # 用来存储路径代价，指从X到达终点G的路径（{X,……G}，简记为{X}）代价，不一定是全局最优。
    ...
    def on_press(self, event):
        ...
        if x < 0 or x > self.x - 1 or y < 0 or y > self.y - 1:
            ...
            if (x, y) not in self.obs:
                ...
                while s != self.s_goal:
                    if self.is_collision(s, self.PARENT[s]): # 遇到了障碍点
                        self.modify(s)  # 从当前点开始重新规划路径
                        continue
                    s = self.PARENT[s]
    ...
    def process_state(self):
        ...
        if k_old < self.h[s]:       # 从当前点的邻居找个路径代价最小的
    ...
    def insert(self, s, h_new):
        ...
        if self.t[s] == 'NEW':
            self.k[s] = h_new
        elif self.t[s] == 'OPEN' or self.t[s] == 'CLOSED':
