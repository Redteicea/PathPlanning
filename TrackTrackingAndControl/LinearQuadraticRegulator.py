"""
轨迹跟踪与控制
 Linear Quadratic Regulator 算法（LQR）
@author: HowGreat
"""
...
N = 100 # 迭代范围
EPS = 1e-4 # 迭代精度
Q = np.eye(3)*3 # 形状为 3*3，对角线为 3，其余元素为0的对角矩阵
R = np.eye(2)*2.
dt = 0.1 # 时间间隔，单位：s
L = 2 # 车辆轴距，单位：m
v = 2 # 初始速度
...
class MyReferencePath:
    ...
    def calc_track_error(self, x, y):
        ...
        yaw = self.refer_path[s, 2]     # 轨迹点的切线方向，航向角
...
def LQR(robot_state, refer_path, s0, A, B, Q, R):
    ...
    K = -np.linalg.pinv(R + B.T @ P @ B) @ B.T @ P @ A  # @ numpy中的@为点乘
    u = K @ x
    u_star = u #u_star = [[v-ref_v, delta-ref_delta]]
    # print(u_star)
    return u_star[0,1]  # numpy数组是可以这样索引的，delta-ref_delta
...
def main():
    ...
    while True:
        ...
        robot_state[3] = ugv.v
        # e:位置跟踪误差, k：目标点曲率, ref_yaw：目标点航向角, s0：目标点索引
        e, k, ref_yaw, s0 = reference_path.calc_track_error(robot_state[0], robot_state[1])
        # ref_delta = math.atan2(L*k, 1)  # 前轮转向角参考值
        ref_delta = math.atan(L*k)  # 前轮转向角参考值
        A, B = ugv.state_space(ref_delta, ref_yaw)
        delta = LQR(robot_state, reference_path.refer_path, s0, A, B, Q, R) # 返回的是控制量误差
        delta = delta + ref_delta   # 误差加上原来的值才是最终的值
