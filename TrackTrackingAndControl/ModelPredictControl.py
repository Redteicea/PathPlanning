"""
轨迹跟踪与控制
 Model Predict Control(MPC)模型预测控制
@author: HowGreat
"""
...
# mpc parameters
NX = 3  # x = x, y, yaw 状态量形状
NU = 2  # u = [v, delta]    控制量形状
T = 8  # horizon length
R = np.diag([0.1, 0.1])  # input cost matrix，生成对角矩阵
...
class MyReferencePath:
    def __init__(self):
        ...
        for i in range(len(self.refer_path)):
            ...
            elif i == (len(self.refer_path) - 1):   # 最后一个
    ...
    def calc_track_error(self, x, y):
        ...
        Returns:
            _type_: 【e跟踪误差，k目标点斜率，yaw目标点航向角（切线方向），s目标点索引号】
        """
        # 寻找参考轨迹最近目标点
    ...
    def calc_ref_trajectory(self, robot_state, dl=1.0):
        ...
        Returns:
            _type_: 【xref：参考状态量，ind：目标点索引号，dref：参考控制量】
        """
...
def linear_mpc_control(xref, x0, delta_ref, ugv):
    ...
    xref: reference point   参考状态量
    x0: initial state   初始状态
    delta_ref: reference steer angle 参考引导角度
    ugv：车辆对象
    """
    # cvxpy # 用于求解凸优化的库
    x = cvxpy.Variable((NX, T + 1)) # 变量，未知数
    ...
    for t in range(T):
        # cvxpy.quad_form(a,b)  # 求 a^T*b*a，a^T为a的转置
    ...
    prob.solve(solver=cvxpy.ECOS, verbose=False)    # 问题求解
...
def main():
    ...
    for i in range(500):
        ...
        # 显示动图
        plt.cla()
        plt.plot(reference_path.refer_path[:, 0], reference_path.refer_path[:, 1], "-.b", linewidth=1.0,
                 label="course")
